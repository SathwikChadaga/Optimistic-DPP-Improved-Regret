...
class OnlineQueueNetwork:
    ...
    def store_metrics(self, queues, planned_edge_rates, actual_edge_rates):
        ...
        self.backlog_at_tt[self.tt] = np.mean(np.sum(queues, axis=1), axis=0)
        self.planned_tran_cost_at_tt[self.tt] = np.mean(planned_edge_rates@self.true_edge_costs, axis=0)
        self.actual_tran_cost_at_tt[self.tt] = np.mean(actual_edge_rates@self.true_edge_costs, axis=0)
